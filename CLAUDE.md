...
| `models.py` | `is_free()` 판정 + `ModelRegistry`(무료 목록 TTL 캐시). |
| `router.py` | `FreeRouter` — 후보 순서 결정 + 429 cooldown 추적. |
| `proxy.py` | FastAPI 앱: `/v1/chat/completions`(스트림 포함), `/v1/models`, `/health`. |
